...
Download `emmc_appsboot.mbn` from https://github.com/chmod-rwx/mainline4lumia.github.io/blob/main/data/emmc_appsboot.mbn. Then, copy `emmc_appsboot.mbn` to your phone's EFIESP folder. Now, unmount mass storage and restart your phone by holding the volume down and power buttons for a few seconds.
...
At this point you should have booted linux on yout device. In this step you'll install a distro on your device. It recommended that you install a distro (such as postmarketOS or Void Linux) on an SD Card and not directly on your device's eMMC.

First, format your SD card to have 1 ext4 partition. Then, extract an armv7 rootfs onto the partition. Alternatively, you can use pmbootstrap to flash the rootfs to the SD card. Insert your SD card into your device. Then get into lk2nd and boot linux with kernel parameters by running `fastboot boot boot.img --cmdline "root=/dev/mmcblk1p1 rootwait"`.
